...
def save_cropped_array_as_nifty_volume(data, image_path, uncropped_image=None):
    """
        save a numpy array as nifty image
        inputs:
            data: a numpy array with shape [Depth, Height, Width]
            image_path: the ouput file path
            uncropped_image: sitk image that is used to set spacing, origin and direction
        outputs: None
        """
    img = sitk.GetImageFromArray(data)
    if uncropped_image is not None:
        spacing = uncropped_image.GetSpacing()
        origin = uncropped_image.GetOrigin()
        direction = uncropped_image.GetDirection()
