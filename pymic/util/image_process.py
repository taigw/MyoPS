...
def get_ND_bounding_box(volume, margin = None):
    ...
    for i in range(len(input_shape)):
        ...
        idx_max.append(int(indxes[i].max() + 1)) # numpy.int64 and int are different sometimes
...
def get_contour(image):
    """get contour from 2D or 3D from binary image

    Args:
        image (ndarray): binary segmentation map

    Returns:
        contour (ndarray): binary contour map
    """    
    image_shape = image.shape
    dim = len(image_shape)
    contour = np.zeros_like(image)
    if dim == 2:
        for h in range(0, image_shape[0] - 1, 1):
            differ_h = image[h, :] - image[h + 1, :]
            contour[h + 1, :][differ_h == -1] = 1
            contour[h, :][differ_h == 1] = 1
        for w in range(0, image_shape[1] - 1, 1):
            differ_w = image[:, w] - image[:, w + 1]
            contour[:, w + 1][differ_w == -1] = 1
            contour[:, w][differ_w == 1] = 1
    elif dim == 3:
        for d in range(0, image_shape[0] - 1, 1):  # TODO: BUG when number of slices is small 
            differ_d = image[d, :, :] - image[d + 1, :, :]
            contour[d + 1, :, :][differ_d == -1] = 1
            contour[d, :, :][differ_d == 1] = 1
        for h in range(0, image_shape[1] - 1, 1):
            differ_h = image[:, h, :] - image[:, h + 1, :]
            contour[:, h + 1, :][differ_h == -1] = 1
            contour[:, h, :][differ_h == 1] = 1
        for w in range(0, image_shape[2] - 1, 1):
            differ_w = image[:, :, w] - image[:, :, w + 1]
            contour[:, :, w + 1][differ_w == -1] = 1
            contour[:, :, w][differ_w == 1] = 1
    return contour