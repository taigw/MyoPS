...
class AttentionExpDiceLoss(nn.Module):
    def __init__(self, n_class, alpha=1, gama=0.0001):
    ...
    def forward(self, input, label):
        '''
        :param input: batch*class*depth*length*height or batch*class*length*height
        :param label: batch*depth*length*height or batch*length*height  注意未经过one_hot，直接输入label即可
        :param dis: batch*class*depth*length*height or batch*class*length*height
        :return:
        '''
        smooth = 1
...
class One_Hot(nn.Module):
    def __init__(self, depth):
        ...
        self.depth = depth
        self.ones = torch.eye(depth)  # torch.sparse.torch.eye
                                             # eye生成depth尺度的单位矩阵

    def forward(self, X_in):
        '''
        :param X_in: batch*depth*length*height or batch*length*height
        :return: batch*class*depth*length*height or batch*class*length*height
        '''
        ...
        out1 = Variable(self.ones.to(X_in.device).index_select(0, X_in))
    ...
    def forward(self, realinput, reallabel, Wl, label_sum):
        ...
        for i in range(1, self.n_class):
            ...
            if torch.sum(mask).item() > 0:
                ...
                ExpLabel = torch.sum(LogLabel+0.01)  # **self.gama
