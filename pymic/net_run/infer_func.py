...
def volume_infer_by_patch(image, net, device, class_num,
    ...
    '''
    # When passing a list, pay attention! Python is pass-by-assignment, so the following three lines of code are essential!
    mini_patch_inshape = copy.deepcopy(mini_patch_inshape)
    mini_patch_outshape = copy.deepcopy(mini_patch_outshape)
    ...
    for d in range(0, padD, stride[0]):
        ...
        for h in range(0, padH, stride[1]):
            ...
            for w in range(0, padW, stride[2]):
                ...
                sub_image = crop_ND_volume_with_bounding_box(image_pad,   # should be image_pad?
    ...
    max_mini_batch = int((total_batch + mini_batch_size -1)/mini_batch_size)  # integer division with ceil
    for mini_batch_idx in range(max_mini_batch):
        ...
        #should we add the following 3 lines?
        mini_batch_crop_start = [0, 0] + margin_lower
        mini_batch_crop_end = list(out_mini_batch[0].shape[0:2]) + [margin_lower[i] + mini_patch_outshape[i] for i in range(img_dim)]
        out_mini_batch = [crop_ND_volume_with_bounding_box(out_mini_batch[i], mini_batch_crop_start, mini_batch_crop_end) for i in range(output_num)]
    ...
    if(max(margin) > 0):
        ...
        crop_end   = [image_pad.shape[2:][i] - margin_upper[i] for i in range(img_dim)]  # or crop_end = [margin_lower[i] + img_shape[i] for i in range(img_dim)]
        crop_end   = [1, class_num] + crop_end
